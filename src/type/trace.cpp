	...
	namespace type {
			...
			)
		{
			...
			if(format) {
				...
				if(length < 0) {
					buffer = TERRA_TYPE_TRACE_EXCEPTION_STRING(TERRA_TYPE_TRACE_EXCEPTION_FORMAT_MALFORMED);
				}
		...
		trace::timestamp(void)
		{
			...
			if(length > 0) {
				...
				result = TERRA_TYPE_TRACE_EXCEPTION_STRING(TERRA_TYPE_TRACE_EXCEPTION_TIMESTAMP_MALFORMED);
			}
